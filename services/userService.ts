...
const jwt=require('jsonwebtoken');
 ...
 export const registration=async(req : Request):Promise<string>=>{
...
export const raise_a_ticket=async(req : Request):Promise<string|boolean>=>{
    return new Promise(async(resolve,reject)=>{
        try
        {   
            const uniqueId = uuidv4();
            ...
            const userid=await client1.query(`SELECT id from users where user_name=$1`,[req.body.created_by])
            req.body.user_id=userid.rows[0].id;
            req.body.status_time=dateTime;
            req.body.raising_time=dateTime;
            req.body.approved_by="not_approved_yet";
            const response=await client1.query(`INSERT INTO "tickets" ("ticket_id","user_id","ticket_description","ticket_status", "raising_time","status_time","created_by","approved_by")  
            VALUES ($1, $2,$3,$4,$5,$6,$7,$8)`,[req.body.ticket_id,req.body.user_id,req.body.ticket_description,req.body.ticket_status,req.body.raising_time,req.body.status_time,req.body.created_by,req.body.approved_by]);
            if(response)
                return resolve(true);
        }
        catch(error:any)
